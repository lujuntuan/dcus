...
#ifdef _WIN32
    int argv;
    LPWSTR* list = CommandLineToArgvW(GetCommandLineW(), &argv);
    if (list) {
        for (int i = 0; i < argv; i++) {
            std::wstring ws(list[i]);
            args.push_back(WCharToChar(ws.c_str()));
        }
        LocalFree(list);
    }
#else
...
template <typename T>
inline T getarg(struct getopt* map, const T& defaults, const char* argv)
{
    return map->has(argv) ? getopt_utils::as<T>(map->at(argv)) : defaults;
}

template <typename T, typename... Args>
inline T getarg(struct getopt* map, const T& defaults, const char* arg0, Args... argv)
{
    T t = getarg<T>(map, defaults, arg0);
    return t == defaults ? getarg<T>(map, defaults, argv...) : t;
}

inline const char* getarg(struct getopt* map, const char* defaults, const char* argv)
{
    return map->has(argv) ? getopt_utils::as<const char*>(map->at(argv)) : defaults;
}

template <typename... Args>
inline const char* getarg(struct getopt* map, const char* defaults, const char* arg0, Args... argv)
{
    const char* t = getarg(map, defaults, arg0);
    return t == defaults ? getarg(map, defaults, argv...) : t;
}
