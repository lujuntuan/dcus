...
inline bool loadClientConfig(httplib::Client& client, const Data& config)
{
    ...
    if (config.value("web_proxy_url").valid() && config.value("web_proxy_port").valid()) {
        ...
        std::string proxyUrl;
        if (Common::getEnvironment("https_proxy").empty()) {
            proxyUrl = Common::getEnvironment("https_proxy");
        } else if (Common::getEnvironment("HTTPS_PROXY").empty()) {
            proxyUrl = Common::getEnvironment("HTTPS_PROXY");
        } else if (Common::getEnvironment("http_proxy").empty()) {
            proxyUrl = Common::getEnvironment("http_proxy");
        } else if (Common::getEnvironment("HTTP_PROXY").empty()) {
            proxyUrl = Common::getEnvironment("HTTP_PROXY");
        }
        if (!proxyUrl.empty()) {
