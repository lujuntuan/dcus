...
#if (defined(DCUS_BUILD_MASTER) && defined(DCUS_USE_DISTRIBUTE_HTTP))
    ...
    const ProgressFunction& progressFunction)
{
    ...
    auto feedToServer = [&statusHelper](httplib::Response& res) {
        ...
        res.set_content(Common::stringSprintf("<h1>Error %s</h1>", std::to_string(res.status)), "text/html");
    };
    ...
    std::string serverFuncName = std::string("/") + DCUS_WEB_PULL_FUNC_NAME + "/[^\\s]*";
    server.Get(serverFuncName, [&](const httplib::Request& req, httplib::Response& res) {
        bool hasPull = false;
        const auto& spVector = Common::stringSplit(req.path, "/");
        if (spVector.size() != 4) {
        ...
        if (spVector[1] != DCUS_WEB_PULL_FUNC_NAME) {
        ...
        for (const auto& file : files) {
            ...
            if (req.has_header("Range")) {
                ...
                res.status = 206;
                range.erase(0, 6);
                const auto& vr = Common::stringSplit(std::move(range), "-");
                if (vr.size() >= 1) {
                    helper->beginPos = Common::stringToInt64(vr[0]);
                }
            }
