...
#if (defined(DCUS_BUILD_WORKER) && defined(DCUS_USE_DOWNLOAD_HTTP)) || (defined(DCUS_BUILD_MASTER) && defined(DCUS_USE_PULL_HTTP))
    ...
    const ProgressFunction& progressFunction)
{
    ...
    for (const auto& file : files) {
        threadPool.enqueue([&, file]() {
            ...
            const auto& clientUrlPair = Common::getIpaddrMethod(file.url());
            ...
            mutex.unlock();
            if (!Common::exists(dir + "/" + file.domain())) {
                Common::mkPath(dir + "/" + file.domain());
            }
            ...
            if (Common::exists(helper->fileName + ".cache")) {
                ...
                if (Common::exists(helper->fileName)) {
            ...
            auto res = client.Get(
                ...
                [&](const char* data, size_t data_length) {
                    ...
                    Common::sleepMilli(DCUS_WEB_TRANSFER_TEST_TIME); // sleep_test
#endif
                    return true;
                },
                [&](uint64_t current, uint64_t total) {
                    ...
                    if (progressFunction) {
                        if (helper->elapsed.get() >= DCUS_WEB_TRANSFER_INTERVAL_MIN || current >= total) {
                            ...
                            transfers.update(std::move(transfer), true);
                            mutex.unlock();
                            if (transferElapsed.get() > DCUS_WEB_TRANSFER_INTERVAL && current < total) {
                                mutex.lock();
            ...
            if (!res) {
                if (res.error() != httplib::Error::Canceled && statusHelper.status.state() != CANCELED) {
                    if (Common::exists(helper->fileName + ".cache")) {
                        ...
                        Common::remove(helper->fileName + ".cache");
                    }
                    if (!helper->exists) {
