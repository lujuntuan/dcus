...
#include "dcus/master/web_queue.h"
#include "commonlib/dir.h"
#include "commonlib/host.h"
    ...
    : Queue(DCUS_QUEUE_ID_WEB)
{
    ...
    if (master_config.value("net_interface").valid()) {
        m_hpr->ipAddress = Common::getIpAddress(master_config.value("net_interface").toString());
    } else {
        m_hpr->ipAddress = Common::getIpAddress();
    }
    if (master_config.value("download_dir").valid()) {
        m_hpr->downloadDir = master_config.value("download_dir").toString();
    } else {
        m_hpr->downloadDir = Common::getTempDir() + "/dcus_master_tmp";
    }
    if (master_config.value("distribute_url").valid()) {
        m_hpr->distributeUrl = master_config.value("distribute_url").toString();
    } else {
        m_hpr->distributeUrl = "0.0.0.0";
    }
    if (master_config.value("distribute_port").valid()) {
        m_hpr->distributePort = master_config.value("distribute_port").toInt();
    } else {
    ...
    if (master_config.value("upgrade_check_interval").valid()) {
        m_hpr->checkInterval = (uint32_t)master_config.value("upgrade_check_interval").toInt();
    }
...
void WebQueue::eventChanged(Event* event)
{
    ...
    case WebEvent::REQ_INIT: {
        ...
        master_engine->postEvent(new MasterEvent(MasterEvent::RES_INIT, Data { { "success", success } }));
        if (success) {
            checkUpgrade();
    ...
    case WebEvent::REQ_DISTRIBUTE: {
        ...
        if (canPull) {
            master_engine->postEvent(new MasterEvent(MasterEvent::REQ_PULL));
        }
        break;
    }
    ...
    case WebEvent::REQ_FEEDBACK: {
        ...
        master_engine->postEvent(new MasterEvent(MasterEvent::RES_FEEDBACK_DONE, Data { { "success", success } }));
        break;
    }
    default:
        break;
...
{
    master_engine->postEvent(new MasterEvent(MasterEvent::RES_ERROR, Data { { "error", errorCode } }));
}

void WebQueue::postIdle()
{
    master_engine->postEvent(new MasterEvent(MasterEvent::REQ_IDLE));
}

void WebQueue::postUpgrade(Upgrade&& upgrade)
{
    lock();
    if (master_engine->upgrade() == upgrade) {
        unlock();
        return;
    }
    unlock();
    master_engine->postEvent(new MasterUpgradeEvent(upgrade));
}

void WebQueue::postCancel(std::string&& id)
{
    lock();
    if (master_engine->cancelId() == id) {
        unlock();
        return;
    }
    unlock();
    master_engine->postEvent(new MasterEvent(MasterEvent::REQ_CANCEL, Data { { "id", id } }));
}
...
{
    bool active = detect();
    if (master_engine->isActive() != active) {
        master_engine->postEvent(new MasterEvent(MasterEvent::REQ_ACTIVE, Data { { "active", active } }));
    }
}
...
{
    lock();
    std::string id = master_engine->upgrade().id();
    std::string cancelId = master_engine->cancelId();
    unlock();
    std::string downloadDir = m_hpr->downloadDir;
    if (all) {
        if (Common::exists(downloadDir)) {
            Common::removeAll(downloadDir);
        }
    } else {
        if (!id.empty()) {
            if (Common::exists(downloadDir + "/" + id)) {
                Common::removeAll(downloadDir + "/" + id);
            }
        }
        if (!cancelId.empty()) {
            if (Common::exists(downloadDir + "/" + cancelId)) {
                Common::removeAll(downloadDir + "/" + cancelId);
            }
        }
    }
}
...
{
    lock();
    id = master_engine->upgrade().id();
    files.clear();
    files.shrink_to_fit();
    for (const Package& package : master_engine->upgrade().packages()) {
...
void WebQueue::download(const std::string& id, const Files& files)
{
    ...
    auto status = Core::pull(
        m_hpr->downloadDir + "/" + id, files, master_config,
        [this]() {
            return m_hpr->workThread.isReadyFinished();
        },
        [](const Transfers& transfers) {
            master_engine->postEvent(new MasterTransferEvent(Transfers(transfers)));
        });
    if (status.state() == Core::SUCCEED) {
        master_engine->postEvent(new MasterEvent(MasterEvent::RES_DOWNLOAD_DONE));
...
void WebQueue::verify(const std::string& id, const Files& files)
{
    ...
    if (status.state() == Core::SUCCEED) {
        master_engine->postEvent(new MasterEvent(MasterEvent::RES_VERIFY_DONE));
    } else if (status.state() == Core::FAILED) {
        postError(status.error());
    }
}
...
void WebQueue::distribute(const std::string& id, const Files& files)
{
    auto status = Core::distribute(
        ...
        m_hpr->distributeUrl, m_hpr->distributePort, m_hpr->downloadDir + "/" + id, files, master_config,
        [this]() {
            return m_hpr->workThread.isReadyFinished();
        },
        [](const Transfers& transfers) {
    ...
    if (status.state() == Core::SUCCEED) {
        master_engine->postEvent(new MasterEvent(MasterEvent::RES_DISTRUBUTE_DONE));
    } else if (status.state() == Core::FAILED) {
        postError(status.error());
    }
}
