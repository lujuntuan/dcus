...
bool HawkbitQueue::init(const WebInit& webInit)
{
    ...
    m_hpr->path = Common::stringSprintf("/%s/controller/v1/%s", webInit.tenant(), webInit.id());
    return true;
}
...
bool HawkbitQueue::detect()
{
    Core::Status status;
    std::string body;
    status = Core::getMessage(m_hpr->url, m_hpr->path, body, master_config, m_hpr->token);
    ...
    int ms = Common::getCurrentTimeSecForString(nextTimeStr, "%H:%M:%S") * 1000;
    ...
    if (!path.empty()) {
        ...
        rootJson["time"] = json11::Json(Common::getCurrentTimeString("%Y%m%dT%H%M%S"));
        lock();
        std::string attributesStr = master_engine->attributes().toStream();
        unlock();
        rootJson["data"] = json11::Json::parse(attributesStr, errorString);
        std::string contentStr;
        json11::Json(std::move(rootJson)).dump(contentStr);
        status = Core::putMessage(m_hpr->url, path, std::move(contentStr), master_config, m_hpr->token);
        if (status.state() != Core::SUCCEED) {
    ...
    status = Core::getMessage(m_hpr->url, path, body, master_config, m_hpr->token);
    if (status.state() != Core::SUCCEED) {
...
bool HawkbitQueue::feedback(const WebFeed& webFeed)
{
    ...
    std::string contentStr;
    json11::Json(std::move(rootJson)).dump(contentStr);
    Core::Status status = Core::postMessage(m_hpr->url, m_hpr->path + urlStr, std::move(contentStr), master_config, m_hpr->token);
    if (status.state() != Core::SUCCEED) {
        postError(status.error());
        return false;
    }
