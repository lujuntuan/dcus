...
Application::Application(int argc, char** argv, const std::string& typeName)
{
    ...
    m_hpr->argc = argc;
    m_hpr->argv = argv;
    m_hpr->exePath = Common::getExePath();
    m_hpr->exeDir = Common::getExeDir(m_hpr->exePath);
    m_hpr->exeName = Common::getExeName(m_hpr->exePath);
...
Data Application::readConfig(const std::string& fileName)
{
    ...
    std::string configPath = Common::getEnvironment("DCUS_CONF_DIR") + "/" + fileName;
    if (!Common::exists(configPath)) {
#ifdef WIN32
        configPath = Common::getEnvironment("ProgramFiles") + "/" + PROJECT_NAME + "/" + expectPath;
        if (!Common::exists(configPath)) {
            configPath = Common::getEnvironment("ProgramFiles(x86)") + "/" + PROJECT_NAME + "/" + expectPath;
#else
...
void Application::loadFlagOnExec(int flag)
{
    if (flag & Application::CHECK_SINGLETON) {
        if (!Common::programCheckSingleton(m_hpr->typeName)) {
    ...
    if (flag & Application::CHECK_TERMINATE) {
        _intance = this;
        Common::programRegisterTerminate([](int reval) {
            _intance->exit(reval);
        });
    }
}
