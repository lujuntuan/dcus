...
class RosWorkerEngine : public WorkerEngine {
public:
    RosWorkerEngine(int argc, char** argv)
...
protected:
    ...
    void respondDetailMessage(msg::DetailMessage::UniquePtr msg_dtl)
    {
        ...
        detailMessage.state() = (MasterState)msg_dtl->state;
        detailMessage.last() = (MasterState)msg_dtl->last;
        ...
        for (const auto& msg_detail : msg_dtl->details) {
            ...
            domain.state() = (WorkerState)msg_detail.domain.state;
            domain.last() = (WorkerState)msg_detail.domain.last;
    ...
    virtual void onStart() override
    {
        ...
        m_rosThread.start([&]() {
            ...
            m_rosControl = m_rosNode->create_subscription<msg::ControlMessage>("/dcus/control", qosConfig,
                std::bind(&RosWorkerEngine::respondControlMessage, this, std::placeholders::_1));
            if (hasSubscibeDetail()) {
                m_rosDetail = m_rosNode->create_subscription<msg::DetailMessage>("/dcus/detail", qosConfig,
                    std::bind(&RosWorkerEngine::respondDetailMessage, this, std::placeholders::_1));
            }
...
WorkerEngine* WorkerEngine::create(int argc, char** argv)
{
    ...
    return new RosWorkerEngine(argc, argv);
}
