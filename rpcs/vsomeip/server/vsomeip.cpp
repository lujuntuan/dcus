...
class VSomeipServerEngine : public ServerEngine {
public:
    VSomeipServerEngine(int argc, char** argv)
...
protected:
    ...
    inline DCusInterfaces::DetailMessage getDetailMessage(uint32_t messageId) const
    {
        ...
        detailMessage.setState_((DCusInterfaces::ServerState::Literal)state());
        detailMessage.setLast((DCusInterfaces::ServerState::Literal)lastState());
...
private:
    virtual void onStart() override
    {
        ...
        public:
            ...
            {
                DCUS_UNUSED(_client);
                VSomeipServerEngine* intance = static_cast<VSomeipServerEngine*>(dcus_server_engine);
                intance->respondDomainMessage(_domainMessage);
            }
        };
        m_commonApiStub = std::make_shared<DCusInterfaceStubImpl>();
        CommonAPI::Runtime::get()->registerService("local", "commonapi.DCusInterfaces", m_commonApiStub, "dcus-server");
    }
...
{
    VSomeipServerEngine engine(argc, argv);
    return engine.exec();
}
