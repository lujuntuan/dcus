...
class VSomeipWorkerEngine : public WorkerEngine {
public:
    VSomeipWorkerEngine(int argc, char** argv)
        : WorkerEngine(argc, argv)
    {
        setInstance(this);
    }
    ~VSomeipWorkerEngine()
...
protected:
    ...
    void respondDetailMessage(const DCusInterfaces::DetailMessage& ci_dtl)
    {
        ...
        detailMessage.state() = (MasterState)ci_dtl.getState_().value_;
        detailMessage.last() = (MasterState)ci_dtl.getLast().value_;
        ...
        for (const auto& ci_detail : ci_dtl.getDetails()) {
            ...
            domain.state() = (WorkerState)ci_detail.getDomain().getState_().value_;
            domain.last() = (WorkerState)ci_detail.getDomain().getLast().value_;
...
private:
    ...
    {
        m_commonApiProxy = CommonAPI::Runtime::get()->buildProxy<DCusInterfacesProxy>("local", "commonapi.DCusInterfaces", "dcus-worker");
    ...
    virtual bool onSendDomainMessage(const Domain& domain, bool discovery) override
    {
        ...
        ci_domain.setState_((DCusInterfaces::WorkerState::Literal)domain.state());
        ci_domain.setLast((DCusInterfaces::WorkerState::Literal)domain.last());
...
WorkerEngine* WorkerEngine::create(int argc, char** argv)
{
    LOG_DEBUG("rpc type: vsomeip");
    return new VSomeipWorkerEngine(argc, argv);
}
