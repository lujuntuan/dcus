...
class FdbusMasterEngine : public MasterEngine, public CBaseServer {
public:
    FdbusMasterEngine(int argc, char** argv)
        : MasterEngine(argc, argv)
        , CBaseServer(DCUS_MASTER_NAME)
    {
        setInstance(this);
        FDB_CONTEXT->init();
    }
...
protected:
    ...
    inline DCusInterfaces::DetailMessage getDetailMessage(uint32_t messageId) const
    {
        ...
        detailMessage.set_state((DCusInterfaces::DetailMessage_MasterState)state());
        detailMessage.set_last((DCusInterfaces::DetailMessage_MasterState)lastState());
        ...
        for (const auto& d : details()) {
            ...
            pb_detail->mutable_domain()->set_state((DCusInterfaces::Domain_WorkerState)d.domain().state());
            pb_detail->mutable_domain()->set_last((DCusInterfaces::Domain_WorkerState)d.domain().last());
...
private:
    ...
    void onInvoke(CBaseJob::Ptr& msg_ref) override
    {
        ...
        if (msgData->code() == DCusInterfaces::TP_DOMAIN_MSG) {
            ...
            domain.state() = (WorkerState)domainMessage.domain().state();
            domain.last() = (WorkerState)domainMessage.domain().last();
...
{
    FdbusMasterEngine engine(argc, argv);
    return engine.exec();
}
