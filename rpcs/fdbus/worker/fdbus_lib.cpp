...
class FdbusWorkerEngine : public WorkerEngine, public CBaseClient {
public:
    FdbusWorkerEngine(int argc, char** argv)
        : WorkerEngine(argc, argv)
        , CBaseClient(this->WorkerEngine::name().c_str())
    {
        setInstance(this);
        FDB_CONTEXT->init();
    }
...
protected:
    ...
    void respondDetailMessage(const DCusInterfaces::DetailMessage& pb_dtl)
    {
        ...
        detailMessage.state() = (MasterState)pb_dtl.state();
        detailMessage.last() = (MasterState)pb_dtl.last();
        ...
        for (int i = 0; i < pb_dtl.details_size(); i++) {
            ...
            domain.state() = (WorkerState)pb_detail.domain().state();
            domain.last() = (WorkerState)pb_detail.domain().last();
...
private:
    ...
    virtual bool onSendDomainMessage(const Domain& domain, bool discovery) override
    {
        ...
        domainMessage.mutable_domain()->set_state((DCusInterfaces::Domain_WorkerState)domain.state());
        domainMessage.mutable_domain()->set_last((DCusInterfaces::Domain_WorkerState)domain.last());
...
WorkerEngine* WorkerEngine::create(int argc, char** argv)
{
    ...
    return new FdbusWorkerEngine(argc, argv);
}
