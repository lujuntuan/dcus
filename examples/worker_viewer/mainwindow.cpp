    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(m_acceptBtn, &QPushButton::clicked, this, []() {
        worker_engine->postDetailAnswer(ANS_ACCEPT);
    });
    connect(m_rejectBtn, &QPushButton::clicked, this, []() {
        worker_engine->postDetailAnswer(ANS_REFUSE);
    });
...
void MainWindow::processDetail(const DetailMessage& detailMessage, bool stateChanged)
{
    ...
    if (Domain::mrStateIsBusy(detailMessage.state()) || Domain::mrStateIsAsk(detailMessage.state())) {
        ui->lineEdit_action->setText(QString::fromStdString(worker_engine->upgrade().id()));
        ui->lineEdit_download->setText(QString::fromStdString(Upgrade::getMethodStr(worker_engine->upgrade().download())));
        ui->lineEdit_deploy->setText(QString::fromStdString(Upgrade::getMethodStr(worker_engine->upgrade().deploy())));
        ui->lineEdit_maintenance->setText(worker_engine->upgrade().maintenance() ? QStringLiteral("true") : QStringLiteral("false"));
...
{
    QMessageBox::about(this, QStringLiteral("About DCus"),
        QStringLiteral("Domain Controller Upgrade Solution\n\nVersion: %1\nCommitID: %2")
