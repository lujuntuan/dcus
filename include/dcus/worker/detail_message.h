...
struct DetailMessage {
    ...
    template <typename T>
    explicit DetailMessage(MasterState state, MasterState last, bool active, int error, float step, float progress, const std::string& messgae, T&& details) noexcept
        : m_state(state)
        , m_last(last)
        , m_active(active)
        , m_error(error)
    ...
    CREATE_PUBLIC_PROPERTY(MasterState, state, MR_UNKNOWN)
    CREATE_PUBLIC_PROPERTY(MasterState, last, MR_UNKNOWN)
